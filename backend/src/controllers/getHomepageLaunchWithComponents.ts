...
export const getHomepageLaunchWithComponents = async (req: Request, res: Response) => {
  try {
    ...
    if (!homepageLaunch) {
      return res.status(404).json({ message: 'Anasayfada gösterilecek lansman bulunamadı.' });
    }
    ...
    const components = await ComponentItem.find({ launchId }).sort({ sequenceNumber: 1 });

    // Eğer bileşen bulunamazsa 404 döneriz
    if (!components.length) {
      return res.status(404).json({ message: 'Bu lansmana ait bileşen bulunamadı.' });
    }
    ...
    res.status(500).json({ message: 'Veriler çekilirken bir hata oluştu.', error });
  }
};
